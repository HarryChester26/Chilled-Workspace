function planesvector
    plane1 = [2 1 3; 1 1 1; 1 3 7];
    plane2 = [2 0 1; 2 0 4; 3 2 1];
    plane3 = [12 6 0; -2 0 2; 0 -1 2];
    ...
    %create our ui
    [ui.plane1Panel, ui.plane1Boxes] = createVecEqnUi([60 65], plane1, colour1);
    [ui.plane2Panel, ui.plane2Boxes] = createVecEqnUi([60 40], plane2, colour2);
    [ui.plane3Panel, ui.plane3Boxes] = createVecEqnUi([60 15], plane3, colour3);
    ...
    function updateButtonCallback(~,~)
        ...
        for i = 1:3 
            for j=1:3
                plane1(i,j) = str2double(ui.plane1Boxes(i,j).String);
                plane2(i,j) = str2double(ui.plane2Boxes(i,j).String);
                plane3(i,j) = str2double(ui.plane3Boxes(i,j).String);
            end
        end
    ...
    function [panel, fields] = createVecEqnUi(position, initData, color)
        ...
        uicontrol(panel,'style','text', 'FontSize', 10, 'HorizontalAlignment', 'left', 'string', "(x,y,z) = (", 'position',[30 0 100 20]);
        uicontrol(panel,'style','text', 'FontSize', 10, 'HorizontalAlignment', 'left', 'string', ") +  s (", 'position',[180 0 50 20]);
        uicontrol(panel,'style','text', 'FontSize', 10, 'HorizontalAlignment', 'left', 'string', ") +  t (", 'position',[310 0 50 20]);
        uicontrol(panel,'style','text', 'FontSize', 10, 'HorizontalAlignment', 'left', 'string', "),  s,t ∈ ℝ", 'position',[440 0 100 20]);
        
        fields = gobjects(3);
        for i=1:3
            for j=1:3
                fields(i,j) = uicontrol(panel,'style','edit', 'FontSize', 10, 'HorizontalAlignment', 'left', 'string', initData(i,j), 'position',[30*j+130*i-68 1 25 20]);
            end
        end
    end
